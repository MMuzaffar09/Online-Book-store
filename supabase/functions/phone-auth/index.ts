...
async function sendOtpSms(phoneNumber: string, otp: string): Promise<void> {
  ...
  if (!smsConfigured) throw new Error("SMS provider not configured.");
...
async function checkRateLimit(phone: string, _ip: string): Promise<boolean> {
  const since = new Date(Date.now() - RATE_LIMIT_WINDOW_MS).toISOString();
  const { count } = await supabase.from("phone_otp_verifications").select("id", { count: "exact", head: true }).eq("phone_number", phone).gt("created_at", since);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "send") {
      ...
      if (await checkRateLimit(normalized, getClientIp(req))) return json({ error: "Too many requests. Please try again later." }, 429);

      const { data: recent } = await supabase.from("phone_otp_verifications").select("last_sent_at").eq("phone_number", normalized).order("created_at", { ascending: false }).limit(1).maybeSingle();
      if (recent) {
        const elapsed = Date.now() - new Date(recent.last_sent_at).getTime();
        if (elapsed < RESEND_COOLDOWN_MS) return json({ error: `Please wait ${Math.ceil((RESEND_COOLDOWN_MS - elapsed) / 1000)}s before requesting a new OTP.` }, 429);
      }
      ...
      const { error: insertError } = await supabase.from("phone_otp_verifications").insert({ phone_number: normalized, otp_hash: otpHash, expires_at: expiresAt.toISOString(), attempts: 0, max_attempts: MAX_ATTEMPTS, verified: false, last_sent_at: now.toISOString() });
      ...
      return json({ message: "OTP sent successfully.", expires_in: 300, ...(testMode ? { test_otp: otp } : {}) });
    }
    ...
    if (action === "verify") {
      ...
      const { data: record, error: queryError } = await supabase.from("phone_otp_verifications").select("id, otp_hash, expires_at, attempts, max_attempts, verified").eq("phone_number", normalized).eq("verified", false).order("created_at", { ascending: false }).limit(1).maybeSingle();
      if (queryError) throw queryError;
      if (!record) return json({ error: "OTP has expired. Please request a new OTP." }, 400);
      if (new Date(record.expires_at).getTime() < Date.now()) return json({ error: "OTP has expired. Please request a new OTP." }, 400);
      if (record.attempts >= record.max_attempts) return json({ error: "Maximum attempts exceeded. Please request a new OTP." }, 400);

      const submittedHash = await hashOtp(code);
      if (submittedHash !== record.otp_hash) {
        const newAttempts = record.attempts + 1;
        await supabase.from("phone_otp_verifications").update({ attempts: newAttempts }).eq("id", record.id);
        const remaining = record.max_attempts - newAttempts;
        if (remaining <= 0) return json({ error: "Maximum attempts exceeded. Please request a new OTP." }, 400);
        return json({ error: `Incorrect OTP. You have ${remaining} attempt${remaining === 1 ? "" : "s"} remaining.` }, 400);
      }

      await supabase.from("phone_otp_verifications").update({ verified: true, attempts: record.attempts + 1 }).eq("id", record.id);
      const phoneDigits = normalized.replace(/\D/g, "");
      const email = `${phoneDigits}@phone.khancreations.app`;
      const fullName = name || "Customer";

      const { error: createError } = await supabase.auth.admin.createUser({ email, password: crypto.randomUUID(), email_confirm: true, user_metadata: { full_name: fullName, phone: normalized } });
      ...
      await supabase.from("profiles").upsert({ id: linkData.user.id, email, full_name: fullName, phone: normalized, role: "customer" });
